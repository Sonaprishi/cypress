describe("Edit Profile", () => {
    ...
    it("Usr FA", () => {
      var file = "../fixture/example.json"
        ...
        cy.get('#ddlAuctiontype').select("Dutch(RA)").wait(6000);
        cy.get('#file1').click().wait(6000);


       
        cy.get('#txtBidDuration').should("be.visible").should("be.disabled");
        cy.get('#txtApprover').should("be.visible").should("not.be.disabled").type("Priya{Enter}");
        ...
        cy.get('#txtCeilingPrice').should("be.visible").should("not.be.disabled").type("700000");
        cy.get('#txtStartingPrice').should("be.visible").should("not.be.disabled").type("50000");
